...
function createPrescription(req, res) {
  ...
  // Check if RecordID and PatientID exist in Record table
  const checkRecordQuery = `SELECT * FROM Record WHERE RecordID = ? AND PatientID = ?`;
  connection.query(checkRecordQuery, [RecordID, PatientID], (checkRecordErr, recordResult) => {
    if (checkRecordErr) {
      console.error("Error checking for existing RecordID and PatientID:", checkRecordErr);
      res.status(500).json({ error: "Internal Server Error, Check if RecordID and PatientID exist in Record table" });
      return;
    }

    if (recordResult.length === 0) {
      // If RecordID and PatientID combination doesn't exist, handle the error
      console.log("RecordID and PatientID combination does not exist in the Record table");
      res.status(400).json({ error: "RecordID and PatientID combination does not exist in Record Table" });
      return;
    }
    ...
    connection.query(checkPrescriptionQuery, [RecordID], (checkPrescriptionErr, prescriptionResult) => {
      ...
      insertPrescription(RecordID, PatientID, DoctorName, Diagnosis, ExtraNotes, res, (insertedPrescriptionID) => {
        if (Drugs.length > 0) {
          // Insert drugs only if there are drugs in the request
...
function generatePrescriptionQuery(joinConditions, whereConditions) {   // Function to generate the common SQL query for retrieving prescriptions
  const sql_query = `
    SELECT prescription.PrescriptionID, prescription.PatientID, prescription.RecordID, prescription.DoctorName, prescription.Diagnosis, prescription.ExtraNotes, 
